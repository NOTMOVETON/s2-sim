...
- [ ] Shift+LMB перемещает камеру (pan)
- [ ] MMB больше не делает pan (ничего не делает или zoom остаётся)
- [ ] Ctrl+Z отменяет последнее действие (добавление, удаление, перемещение)
- [ ] До 50 операций в стеке undo
- [ ] Shift+клик на примитив добавляет его к выделению (подсветка emissive)
- [ ] Ctrl+C копирует выделенные примитивы (один или группу)
- [ ] Ctrl+V вставляет со смещением +0.5 по X и Y
- [ ] Delete / Backspace удаляет выделенные примитивы
- [ ] Copy/Paste/Delete не работает с агентами
- [ ] После undo геометрия на сервере обновляется (sendGeometryToServer)
