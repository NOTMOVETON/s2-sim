...
- [ ] `GET /api/scenes` возвращает список `.yaml` файлов из `s2_config/scenes/`
- [ ] Панель "Scenes" в UI показывает список с кнопками "Загрузить"
- [ ] При загрузке сцены показывается overlay "Загрузка..."
- [ ] После загрузки симуляция стартует с нуля (время = 0, агенты на начальных позициях)
- [ ] Старые меши очищены, новые геометрия и агенты отображаются
- [ ] "Сохранить как..." создаёт новый YAML-файл в `s2_config/scenes/`
- [ ] "Новая сцена" создаёт пустую сцену с одним агентом и загружает её
- [ ] Активная сцена помечается в списке

## Тесты

- `SimEngine::reload_scene` — тест: загрузить сцену А, reload на сцену Б, агенты Б присутствуют, агенты А отсутствуют
- `SceneWriter::save_as` — создаёт файл с правильным содержимым
