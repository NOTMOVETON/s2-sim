...
- [ ] Агент с плагином `gravity` падает вниз при z > floor
- [ ] Агент приземляется на пол (box z≈0) и не проваливается
- [ ] Агент приземляется на платформу и стоит на ней
- [ ] Если агент съезжает с платформы — падает на пол
- [ ] Вертикальная скорость не превышает `max_fall_speed`
- [ ] `to_json()` возвращает `grounded: true` когда агент на поверхности
- [ ] Плагин не влияет на горизонтальное движение (diff_drive работает как обычно)
- [ ] Агент без плагина `gravity` — поведение не меняется (z остаётся неизменным)
