/**
 * @file conveyor_wire_controller.cpp
 * ConveyorWireController -- реализация wire-контроллера конвейера.
 *
 * Аналогичен DoorWireController, но с реакциями "stop", "reverse", "start".
 * Доставляет через cmd::Interact на актора (D-13).
 */
...
void ConveyorWireController::from_config(const YAML::Node& node)
{
...
void ConveyorWireController::update(double /*dt*/, Agent& agent, const PluginContext& ctx)
{
...
std::string ConveyorWireController::to_json() const
{
    nlohmann::json j;
    j["type"]      = "conveyor_wire_controller";
